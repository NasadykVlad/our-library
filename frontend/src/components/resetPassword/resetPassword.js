...
const ResetPassword = ({t}) => {
    ...
    const sendToken = () => {
        let input2 = document.querySelector('#token-login').value
        let input3 = document.querySelector('#password-login').value
    ...
    return (
        <div className="resetPassword">
            ...
            <Form noValidate onSubmit={e => e.preventDefault()}>
                <Form.Group controlId="formBasicEmail114" className={statusEmail === 'mail-send' ? 'mail-send mb-3' : 'mb-3'}>
                    ...
                    <Button style={{'marginTop': '10px'}} variant="secondary" onClick={() => resetPassword()} type="submit">{t('send')}</Button>
                </Form.Group>
                ...
                {statusEmail === 'mail-send' ?
                    ...
                    <Form.Group className="mb-3" controlId="formBasicPassword116">
                        ...
                        {errorsTokenPassword.length > 0
                            ...
                            : rightReset ? <p style={{'marginTop': '5px', 'marginBottom': '0px', 'color': 'green'}}>{t('password-change')}.</p> : ''
                        }

                        <Button style={{'marginTop': '10px'}} variant="secondary" onClick={() => sendToken()} type="submit">{t('change-password')}</Button>
                    </Form.Group>
                    </div> : ''
                }
            </Form>
